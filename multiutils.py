...
class MultiProcessor(DataProcessor):
    """Processor for the multi data sets"""
    ...
    def _create_examples(self, lines, set_type):
        ...
        def one_hot_encoder( index):
            label_list = self.get_labels(6)
            labels = [float(0)] * len(label_list)
            labels[index] = float(1)
            return labels
        ...
        if(set_type!="predict"):
            for (i, line) in enumerate(lines):
                ...
                label = line[1]
                examples.append(
                    InputExample(guid=guid, text_a=text_a, text_b=None, label=one_hot_encoder(int(label))))
        else:
            for (i, line) in enumerate(lines):
                guid = "%s-%s" % (set_type, i)
                text_a = line[3]
